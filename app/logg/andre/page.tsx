...
export default function AndreLoggPage() {
  const router = useRouter()
  const { t, language } = useLanguage()
  const currentLanguage: UiLanguage =
    language === "en" || language === "es" ? language : "no"
  const text = otherLogTexts[currentLanguage]
  ...
  async function loadPage(selectedVenue: string) {
    ...
    try {
      ...
      const otherList = (listData as Array<{ id: string; name: string }>).find(
        (list) => isOtherTasksList(list.name)
      )

      if (!otherList) {
        setFeil(`${t("error")}: ${t("otherTasks")} ${text.notFoundForVenue}`)
        return
      }

      const listId = otherList.id
