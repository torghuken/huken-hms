...
export default function StengingLoggPage() {
  ...
  async function loadPage(selectedVenue: string) {
    ...
    try {
      ...
      const listRes = await fetch(
        `${url}/rest/v1/task_lists?select=id,name,venue_id&name=ilike.*stenging*&venue_id=eq.${selectedVenue}`,
        {
          headers: {
            apikey: key,
      ...
      if (!listRes.ok || !listData.length) {
        setFeil("Fant ikke oppgavetypen Stenging for valgt sted")
        return
      }
