  ...
  return (
    <main className="min-h-screen bg-black px-6 pt-20 text-white">
      <div className="mx-auto flex max-w-md flex-col items-center">
        ...
        <div className="flex w-full flex-col items-center gap-4">
          {employees.map((emp) => (
            <button
              ...
              className="h-16 w-[90%] rounded-2xl bg-white text-xl font-semibold text-black shadow-lg transition active:scale-95"
            >
              {emp.name}
            </button>
          ))}
        ...
        {employees.length === 0 && !feil && (
          <p className="mt-8 text-zinc-400">{text.noEmployeesFound}</p>
        )}
      </div>
    </main>
  )
